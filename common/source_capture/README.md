...
| File | Role |
|------------------------------|----------------------------------------------------------------|
| `source_capture.js`          | Browser client (`window.SourceCapture`); transport + log buffer |
| `source_capture_server.py`   | Python 3 stdlib receiver; validates + writes files              |
| `source_capture_server.zsh`  | Conventions-compliant launcher that spawns the receiver         |
...
common/source_capture/source_capture_server.zsh

# Custom port / token:
common/source_capture/source_capture_server.zsh --port 9000 --token "$(uuidgen)"
```
